...
// ─── VARIABLE INJECTION ────────────────────────────────────────────
function injectVariables(text, state) {
    ...
    if (cycle > 1 && intensity) {
        if (intensity.prefix && !result.startsWith('['))
            result = intensity.prefix + result;
        if (intensity.suffix)
            result = result + intensity.suffix;
    }
    return result;
}
// ─── FRESHNESS SCORING ─────────────────────────────────────────────
function getItemFreshness(item, state) {
    ...
    if (distance < effectiveCooldown)
        return 0;
    return Math.min(1.0, (distance - effectiveCooldown) / 10);
}
// ─── VARIATION SELECTION ───────────────────────────────────────────
function selectVariation(item, state) {
    if (item.template)
...
function evaluateTriggerCondition(condition, state, intent) {
    const cond = condition;
    // Intent-based conditions
    if (cond.includes('user_rebels') && intent !== 'rebellion')
        return false;
    if (cond.includes('user_seeks_praise') && intent !== 'praise_seeking')
        return false;
    if (cond.includes('user_requests_privilege') && intent !== 'negotiation')
        return false;
    if (cond.includes('user_asks_for_favor') && intent !== 'negotiation')
        return false;
    if (cond.includes('timer_expired') && intent !== 'timer_expired')
        return false;
    if (cond.includes('user_requests_relock') && intent !== 'relock')
        return false;
    if (cond.includes('user_returns_after_dismissal') && intent !== 'relock')
        return false;
    if (cond.includes('user_mentions_freedom') && intent !== 'rebellion')
        return false;
    if (cond.includes('user_seeks_validation') && intent !== 'praise_seeking')
        return false;
    if (cond.includes('deal_was_broken') && intent !== 'normal')
        return false;
    // Tagesform conditions
    if (cond.includes('tagesform')) {
        const tfMatch = cond.match(/tagesform\s*==\s*['"]([^'"]+)['"]/);
        if (tfMatch && state.tagesform !== tfMatch[1])
            return false;
    }
    ...
    if (phaseMatch) {
        ...
        if (state.currentPhase < requiredPhase)
            return false;
    }
    ...
    if (loopMatch) {
        ...
        if (state.loopCycle <= requiredLoop)
            return false;
    }
    return true;
}
// ─── MAIN CONTENT SELECTION ────────────────────────────────────────
export function selectContent(state, userMessage, intent) {
    if (!manifest)
        throw new Error('Manifest not loaded');
    const availablePools = [];
    for (const [poolName, pool] of Object.entries(manifest)) {
        // Phase filter
        if (typeof pool.phase === 'number' && pool.phase !== state.currentPhase)
            continue;
        // 'any' phase pools are always available
        // Trigger condition evaluation
        if (pool.triggerCondition) {
            if (!evaluateTriggerCondition(pool.triggerCondition, state, intent))
                continue;
        }
        ...
        if (poolMoodTags.length > 0) {
            ...
            if (!poolMoodTags.includes(currentMoodTag))
                continue;
        }
    ...
    for (const { poolName, pool } of availablePools) {
        for (const item of pool.items) {
            ...
            if (pool.tags.includes('sissy') && (state.sissyLevel || 0) > 5)
                score *= 1.3;
            if (pool.tags.includes('nuria') && (state.obedienceScore || 50) < 30)
                score *= 1.2;
            if (pool.tags.includes('chastity') && state.chastityStatus === 'caged')
                score *= 1.1;
            if (pool.tags.includes('maid') && (state.maidLevel || 0) > 3)
                score *= 1.25;
            if (pool.tags.includes('slut') && (state.slutLevel || 0) > 3)
                score *= 1.25;
            if (pool.tags.includes('adhd') && state.loopCycle > 2)
                score *= 1.3;
            // Loop cycle boosts
            if (state.loopCycle > 1 && pool.tags.includes('endgame'))
                score *= 1.4;
            if (state.loopCycle > 1 && pool.tags.includes('loop'))
                score *= 1.5;
            if (state.loopCycle >= 2 && pool.tags.includes('betrayal'))
                score *= 1.2;
            if (state.loopCycle >= 3 && pool.tags.includes('romantic'))
                score *= 1.15;
            // Anti-clustering: reduce score for recently used pools
            const recentIds = state.contentFingerprint.slice(-5);
            const samePoolRecent = recentIds.filter(fid => pool.items.some(i => i.id === fid)).length;
            if (samePoolRecent > 2)
                score *= 0.6;
...
export function determineIntent(userMessage, state) {
...
// ─── TAGESFORM ROTATION ────────────────────────────────────────────
export function rotateTagesform(loopCycle) {
...
export async function buildSystemPrompt(basePromptPath) {
