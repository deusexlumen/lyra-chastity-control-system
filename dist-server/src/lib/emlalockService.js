function isEmlalockError(body) {
    if (typeof body !== 'object' || body === null)
        return false;
    return 'error' in body && body.error !== undefined;
}
export function parseEmlalockKeys(combined) {
...
export async function applyPenalty(minutes, keys, fetchImpl = async (url) => {
...
export async function queuePenalty(profile, keys, minutes, fetchImpl) {
    if (minutes === 0)
...
export async function processQueue(profile, keys, fetchImpl) {
    const remaining = [];
