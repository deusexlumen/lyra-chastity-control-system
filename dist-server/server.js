...
// ═══════════════════════════════════════════════════════════════════
async function sendEmail(subject, text, isAmbush = false) {
...
async function triggerEmailAmbush(profile) {
...
app.post("/api/chat", async (req, res) => {
    try {
        ...
        const historyText = db.chat_history.slice(-10).map((m) => `${m.role}: ${m.content}`).join("\n");
